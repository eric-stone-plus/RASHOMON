...
- The adversarial review instrument produces adversarial residuals through independent testimony, anonymous cross-examination, evidence gates, and dual verdict.
- The governance-boundary instrument selects and enforces the runtime path for producing, validating, measuring, preserving, closing, waiving, blocking, or escalating residuals.
...
7. Send action-relevant residuals to the governance-boundary action gate.
...
- `escalated`: the residual affects an action boundary and must pass through governance-boundary review.
...
Residual-first architecture treats every instrument as an instrument, not as a
standing authority.

Use the adversarial review instrument when a consequential conclusion may
affect code, protocol, or irreversible action and adversarial review is needed
before trust or action.

Use direct runtime evidence when a claim is executable; behavior outranks
testimony, with adversarial review added only when interpretation still
matters.

Use the governance-boundary instrument when the residual is at a governance or
action boundary because routing, authorization, and protected writes are
runtime concerns.
...
same-model adversarial review can surface valuable behavioral residuals while
still losing
...
- Cost burn: debate continues after no useful residuals appear. Control: governance-boundary resource, route, and governance gates.
...
- `instrument`: the producing route, such as `adversarial-review`, `direct-evidence`, `governance-boundary`, or `human-review`.
...
- `governance_decision`: the governance-boundary instrument's decision when a runtime gate has evaluated the trace.
...
review instrument produces them under different pressure regimes; the
governance-boundary instrument decides whether a high-risk residual is closed
enough for action.
...
- The adversarial review instrument must produce adversarial residuals with source provenance from R1/R2 and an R3 closure ledger for action-blocking findings.
...
- The governance-boundary instrument must select the next evidence route, consume the trace, and pass, review, block, escalate, or record that no action boundary applies.
...
The governance-boundary instrument may bind route decision, trace validation,
quality metrics, execution evidence, and action scope into an Action Packet.
The packet is a consumer-side evidence bundle; it does not replace the residual
trace schema. For adversarial review near protected or irreversible boundaries,
execution evidence includes complete phase dispatch ledgers so an incomplete
debate cannot be laundered into a valid-looking residual trace.
...
This is the shared conceptual contract: RASHOMON asks what residuals mean; the
adversarial review instrument produces them under its pressure regime; the
governance-boundary instrument decides what may be done with them.
