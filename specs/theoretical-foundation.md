...
reasoning. RASHOMON therefore treats same-model adversarial review as
controlled inference-time perturbation, not as independent confirmation or
...
- The adversarial review instrument produces adversarial residual traces.
- Direct evidence produces execution or source-verification traces.
- The governance-boundary instrument consumes traces and blocks unsafe action.
...
- False consensus: correlated perspectives agree around unsupported claims.
  Required control: evidence gates, diversity notes, and governance-boundary
  closure checks.
...
- Closure laundering: a verdict detects a problem and is then used as
  permission to act.
  Required control: governance-boundary residual closure enforcement.
...
10. Governance-boundary enforcement currently depends on available verdict
   trails and JSON ledgers; earlier traces may only warn.
