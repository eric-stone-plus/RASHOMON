...
These metrics are derived from the trace after production. The adversarial
review instrument, or any other producer, may not close the loop by merely
asserting that its own output is high quality.
...
This advisory gate does not replace governance-boundary authorization. It
helps operators decide whether the residual trace is informative enough to
rely on.
...
RASHOMON defines metric meaning. The governance-boundary instrument or another
host measurement layer may implement the calculation. The adversarial review
instrument should produce enough trace material for the metrics to be derived,
but it should not be trusted as the authority on its own trace quality.
...
and residual trace. The governance-boundary instrument's Action Packet records
that binding. RASHOMON defines the meaning of the metrics; the
governance-boundary instrument owns the packet format and the runtime boundary
decision.
...
- same-model adversarial review versus isolated self-correction
- convergence versus direct evidence
...
baseline, the governance-boundary instrument should route future work
differently.

The governance-boundary instrument may implement this as residual trial
scoring: a bounded profile over residual yield, closure strength, manifest
strength, risk penalty, and cost-normalized yield. The score is not truth
probability. It is a calibration signal for whether adversarial review, direct
evidence, or human review was the right route for similar future work.
...
using it again, rerouting to a cheaper baseline, escalating to adversarial
review or human review, or blocking when high-risk residuals remain open.
...
defines the meaning of residual quality signals, while the governance-boundary
instrument owns the machine-readable report format and validator.
...
This is especially important for same-model adversarial review. A
single-base-model debate may expose useful behavioral residuals, but it must
still earn its cost against isolated self-correction, direct verification,
human review, or heterogeneous review when those routes exist.

RASHOMON defines the semantic claim: residual value is comparative, not
self-certifying. The governance-boundary instrument owns the concrete
route-baseline report format and validator.
...
boundary, then the governance-boundary instrument compares residual scores and
risk deltas. This avoids LLM-as-judge preference voting and asks whether the
route exposed better residual evidence.
...
is used for policy. The governance-boundary instrument owns the concrete route
experiment manifest and review formats and validators.
...
RASHOMON defines the semantics of outcome feedback. The governance-boundary
instrument owns the concrete outcome-ledger format and validator.
...
The governance-boundary instrument owns the concrete route-policy report
format and validator.

## 15. Route Execution Feedback

Residual quality can look strong while the route that produced it is
operationally unreliable. For adversarial review, a protected-boundary trace is
not enough if R1, R2, or R3 dispatch evidence is missing, degraded, blocked, or
invalid.
...
part of residual evidence. The governance-boundary instrument owns the concrete
route execution report format and validator.
...
when their provenance survives synthesis. The governance-boundary instrument
owns the concrete chain validator.
