...
- **Residual Route Baseline Report**: the governance-boundary instrument's
  comparison between a target route group and a same-boundary baseline route
  group. It asks whether the target route earned its cost against a cheaper,
  stronger, or more direct evidence path.
- **Residual Route Pairing Report**: the governance-boundary instrument's
  same-question comparison between target and baseline traces for the same
  action boundary. It uses residual scoring deltas rather than LLM judge
  voting, so a route is challenged by what it exposed, not by which answer
  sounded better.
...
- **Residual Route Experiment Review**: non-authorizing comparison between a
  ...
  traces.
- **Residual Route Change Proposal**: non-authorizing governance-boundary
  artifact that turns a route policy report into maintainer-review candidate
  changes, required gates, and affected documentation or host-overlay paths. It
  does not edit routing rules.
...
contract is Residual Trace plus governance-boundary residual closure.
