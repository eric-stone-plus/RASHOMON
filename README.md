...
> Akutagawa Ryunosuke《In a Grove》(1922), Akira Kurosawa《Rashomon》(1950), multi-perspective truth-seeking
...
instruments consuming the residual ontology operate on.
...
A governance-boundary instrument may package a route decision, residual trace,
validation result, and quality metrics into an Action Packet for a proposed
action.
When trace cohorts are used to justify future route policy, the
governance-boundary instrument can also bind them to same-question route
pairing reports,
same-boundary baseline reports, a pre-run route experiment manifest, and a
post-run experiment review so baseline, pairing, and stopping-rule evidence
remains inspectable.
...
- **Adversarial review instrument**: structured adversarial residual exposure.
  It forces independent testimony, cross-examination, evidence validation, and
  dual verdict.

- **Governance-boundary instrument**: host-side operation layer. It selects the
  runtime route, checks authorization, measures trace quality, and prevents
  unresolved high-risk residuals from crossing protected action boundaries.

The instruments may be composed by a host runtime, but their authority remains
separate. The adversarial review instrument exposes adversarial residuals; the
governance-boundary instrument decides which route is required and what may be
...
- **Akutagawa Ryunosuke**《In a Grove》(1922) — seven testimonies, one death, no resolution
- **Akira Kurosawa**《Rashomon》(1950) — Venice Golden Lion, Academy Award for Best Foreign Language Film
