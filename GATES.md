...
RASHOMON's five mandatory gates. **Parallel execution, hm-operated.** Shōmon gate layer makes a rapid "does this need adversarial review?" determination; if passed, the full pipeline executes.

Update: Kennōmon (Constitution Gate) added as Gate 5, ratified 2026-06-19 — architecture gate enforced by the governance-boundary instrument's Protected-Write Guard. Shōmon now has two layers: the Shōmon gate, where hm makes a rapid judgment about whether the output is consequential; and Shōmon execution, where the full review protocol runs with hm per-phase synchronous veto.

## Gate 1: Amamon (Rain Gate) · Ambiguity Gate
...
**Design rationale**: Spending 30 minutes running a full adversarial review only to discover everyone misunderstood the question is far more expensive than asking one clarifying question.
...
**Mechanical enforcement**: Every comparative statement must begin with `[Kyōmon ✓]` followed by verification evidence.

**Operator**: hm (xhigh reasoning), executed in parallel in Phase -1.

**Naming**: kagami = mirror. Yata no Kagami, the sacred mirror that reflects truth without interpretation.

## Gate 3: Shōmon · Testimony Gate
...
**Failure mode**: Unnecessary adversarial review (trivial query getting full debate) or missed adversarial review (conclusion user may rely on but not debated).

**Trigger**: User question may produce a conclusion the user will rely on.
**Action**: Rapid judgment. Consequential output enters the pipeline; non-consequential lookups such as "where is this file" skip adversarial review.
**Do not self-judge simplicity**: "This is too simple for adversarial review" = violation. Simple tasks hiding subtle errors are exactly why adversarial review exists.
...
## Gate 4: Kan'nukimon · Anti-Drift Gate
...
## Gate 5: Kennōmon · Architecture Gate
...
**Failure mode**: hm makes unilateral architecture changes (protocol rewrites, gate modifications, README restructuring) without adversarial review. The constrained party is also the enforcer — text rules are self-read, self-interpreted, self-enforced.
...
**Action**: The governance-boundary instrument's Protected-Write Guard
intercepts the write attempt and checks for a valid verdict trail. When the
verdict contains a residual trace, the Guard also checks that high-risk
residuals are closed, blocked, waived, or not applicable with evidence and
scope. Without adequate trail or closure, the protected write is blocked.

**Provenance**: Enforced by the governance-boundary instrument's Protected-Write
Guard and specified in the governance-boundary instrument's residual-closure
specification. The gate is named for ken (constitution, fundamental law) + mon
(gate).
...
then presents the final result. Any push remains behind the governance-boundary
instrument's Authorization Gate.
...
- **Kennōmon is enforced externally by the governance-boundary instrument's
  Protected-Write Guard**, not by hm self-enforcement. This breaks the
  self-execution circularity (hm enforcing rules on itself).

*GATES.md — Five gates with Kennōmon architecture gate (enforced by the
governance-boundary instrument's Protected-Write Guard), 2026-06-19, updated
2026-06-22*
