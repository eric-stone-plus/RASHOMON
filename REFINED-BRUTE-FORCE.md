...
>
> Historical label for the adversarial review instrument's architecture,
> recorded from a 2026-06-09 meta-review run.

## Origin

Coined by **omp** (R1, 2026-06-09 meta-review) during a discussion of *«при достаточной тяге и кирпич полетит»*. The user proposed adversarial review was brute-force aesthetics — Soviet aviation style: compensate for lack of refinement with raw power. Five agents debated whether adversarial review is truly brute force or something more nuanced.
...
**What is brute force about adversarial review**:
...
**What is refined about adversarial review**:
- Five Gates: targeted failure-mode defense, not "run more checks"
  - Amamon (Rain Gate): ambiguity detection followed by clarification
  - Kyōmon (Mirror Gate): bidirectional grep + evidence anchoring + directional explicitness
  - Shōmon (Testimony Gate): cross-examination topology with recusal rules
  - Kan'nukimon (Anti-Drift Gate): three-layer anti-drift prompt engineering
  - Kennōmon (Architecture Gate): protected-write check enforced by the
    governance-boundary instrument's Protected-Write Guard, backed by the
    governance-boundary instrument's residual-closure specification
...
- Authorization Gate perimeter: session-level push gate
...
The MiG-25 (NATO: Foxbat) is the canonical example. While American engineers pursued titanium airframes and exquisite aerodynamics for their Mach-3 interceptors, Soviet engineers faced a different set of constraints. Titanium was expensive and difficult to work with given Soviet metallurgy. The solution was not better materials — it was bigger engines. The MiG-25's Tumansky R-15 turbojets produced 11 tonnes of thrust each. Two of them, side by side, propelled a largely stainless-steel airframe to Mach 3.2. The aircraft was heavy, aerodynamically crude, and drank fuel at an alarming rate — but it *worked*. It intercepted SR-71 Blackbirds, outran missiles, and forced the United States to redesign its next-generation fighter program in response to a threat that turned out to be largely illusory. The Foxbat was a brick. It flew because the engines were enormous.
...
**The adversarial review instrument sits in the same lineage.** It does not
rely on a single model's elegant reasoning. It spends extra review effort to
surface blind spots through structured redundancy. The difference: the Soviet
brick was genuinely just a brick with massive engines. The adversarial review
instrument's brick has fins (the Five Gates, each targeting a specific failure
mode) and a flight computer (the Orchestration-Oversight Separation). The thrust is still expensive, but the
